...
function createContent(config){
    ...
    // 生成随机数
    var randData = s2as(32);
    // console.log("随机数:"+randData);
    var mpiKey = process.env.VUE_APP_APPKEY;
    // 获取过程密钥
    var processKey = encryptByDES(mpiKey, randData, true);
    // console.log("过程秘钥:"+processKey);
    // 加密ActionInfo
    // console.log('json明文:'+JSON.stringify(config));
    var src = strToHexCharCode(JSON.stringify(config));
...
//解析数据
function resolveContent(actionInfo) {
    var mpiKey = process.env.VUE_APP_APPKEY;
    // 获取随机数
    var randData = actionInfo.substring(0, 32);
    // 获取应用密文
    var singData = actionInfo.substring(32, actionInfo.length);
    // 获取过程密钥
    var processKey = encryptByDES(mpiKey, randData, true);
    // 解密singData
    var actionInfoString = decryptByDES(processKey, singData);
    // console.log('actionInfoString:'+actionInfoString);
    var json = JSON.parse(actionInfoString);
...
function padding80(data) {
    ...
    var padstr = "";
    for (var i = 0; i < padlen - 1; i++) padstr += "00";
    data = data + "80" + padstr;
...
function encryptByDES(mpiKey, message, isSessionKey) {
    mpiKey = mpiKey + mpiKey.substring(0, 16);
    //console.log("秘钥："+mpiKey);
    //转16进制
    // console.log(message);
    //过程密钥不做填充
    if (!isSessionKey) {
        message = padding80(message);
        // console.log('padding:'+message);
    }

    message = CryptoJS.enc.Hex.parse(message);

    var keyHex = CryptoJS.enc.Hex.parse(mpiKey);
...
function decryptByDES(mpiKey, ciphertext) {
    ...
    var num = data.lastIndexOf("80");
    // 截取actionInfoString
    // 最后一个'80'出现的位置
    if (num != -1) {
        data = data.substring(0, num);
    }
    var result = hexCharCodeToStr(data);
    return result;
}
...
export { createContent , resolveContent }